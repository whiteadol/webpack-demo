...
(() => {
  ...
  var require_device_uuid = __commonJS({
    "node_modules/device-uuid/lib/device-uuid.js"(exports) {
      (function(exports2) {
        ...
        var DeviceUUID2 = function(options) {
        ...
        exports2.DeviceUUID = DeviceUUID2;
        return new DeviceUUID2(navigator.userAgent);
      })(exports);
    }
  });
            ...
            if (sample) {
              sample.forEach((data, idx2) => {
                ...
                p.addEventListener("click", async (e) => {
                  e.currentTarget.parentNode.childNodes.forEach((one2) => {
                    if (one2.nodeType === 1)
  ...
  var DeviceUUID = require_device_uuid2();
  ...
  import_notiflix.default.Notify.init({
    messageFontSize: "2vh",
    messageColor: "#0db5e7",
  ...
  document.addEventListener("DOMContentLoaded", async () => {
    ...
    if ((isAndroid || isiPhone) && maxTouchPoints > 1) {
      ...
      if (browser !== "Safari") {
        ...
        import_notiflix.default.Notify.info(`\uBC30\uD130\uB9AC \uCDA9\uC804 \uC911 - ${charging ? "Y" : "N"}`);
        import_notiflix.default.Notify.info(`\uBC30\uD130\uB9AC \uC0C1\uD0DC - ${level * 100}%`);
        if (!charging && level <= 0.5) {
          alert("\uC601\uC0C1\uBD84\uC11D\uC744 \uC704\uD574 \uCD5C\uC18C 80% \uC774\uC0C1\uC758 \uBC30\uD130\uB9AC\uB97C \uC720\uC9C0\uD574 \uC8FC\uC2ED\uC2DC\uC624.");
        }
        battery.onchargingchange = (ev) => import_notiflix.default.Notify.info(`\uBC30\uD130\uB9AC \uCDA9\uC804 \uC911 - ${ev.target.charging ? "Y" : "N"}`);
        battery.onlevelchange = (ev) => import_notiflix.default.Notify.info(`\uBC30\uD130\uB9AC \uC0C1\uD0DC - ${ev.target.level * 100}%`);
      }
    } else {
      const { protocol, host, port, origin } = window.location;
      const qrcodeText = encodeURI(`${origin}/exam`);
      const qrResult = await (0, import_qrcode.toDataURL)(qrcodeText);
      document.getElementById("qrcode").src = qrResult;
    }
    window.addEventListener("online", (e) => {
      import_notiflix.default.Notify.warning("\uC778\uD130\uB137 \uC5F0\uACB0\uC774 \uBCF5\uAD6C\uB418\uC5C8\uC2B5\uB2C8\uB2E4.");
    });
    window.addEventListener("offline", (e) => {
      import_notiflix.default.Notify.warning("\uC778\uD130\uB137 \uC5F0\uACB0\uC774 \uB04A\uACBC\uC2B5\uB2C8\uB2E4.");
    });
    import_notiflix.default.Loading.hourglass("Python Module Loading.....");
    const indexDBed = new IndexDBed();
    await indexDBed.Init();
    const results = await indexDBed.DataSelect();
    const pyThonMirror = new PyMirror({ el: document.getElementById("code"), output: document.getElementById(`output`) });
    ...
    tabEl.forEach(
      (oneTab) => oneTab.addEventListener("shown.mdb.tab", (e) => {
        setTimeout(() => {
          TabCntr(e);
        }, 300);
      })
