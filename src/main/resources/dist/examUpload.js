...
* Python Version - 3.10.2
  ...
  var ExcelParse = class {
    constructor({ workBook }) {
      this.sheets = workBook.SheetNames;
      this.sheetJsonStore = this.sheets.map((oneSheet, idx) => {
        return {
          examType: this.sheets[idx],
          data: XLSX.utils.sheet_to_json(workBook.Sheets[oneSheet])
        };
      }).map((obj) => this.ReturnTypeFrom(obj)).flat().map((one, idx) => {
        one["probNum"] = idx + 1;
        return one;
      });
    }
    ...
    ExamType_0(examProb) {
      return examProb.flatMap((row, idx, array) => {
        ...
        examForm["sample"] = oneProb.flatMap((one, idx2) => idx2 > 0 ? Object.values(one)[0] : []);
        return examForm;
      });
    }
    /**
    ...
    ExamType_1(examProb) {
      return examProb.flatMap((row, idx, array) => {
        ...
        examForm["sample"] = oneProb.flatMap((one, idx2) => idx2 > 2 ? Object.values(one)[0] : []);
        return examForm;
      });
    }
    ExamType_2(examProb) {
  ...
  document.addEventListener("DOMContentLoaded", async () => {
    ...
    document.getElementById("excelFile").addEventListener("change", (event2) => {
      ...
      reader.onload = function() {
        ...
        question.question.filter((oneProb) => oneProb["questType"] === 3).forEach((one) => {
          ...
          btn.addEventListener("click", async (e) => {
            const probNum2 = e.target.dataset.probNum;
            console.log(`\uD074\uB9AD\uD55C \uC8FC\uAD00\uC2DD \uBC88\uD638 :: ${probNum2}`);
            ...
            const results2 = await indexDBed.DataSelect(probNum2);
            question.CreateSubjectiveQuestion(probNum2);
            if (results2.length > 0) {
              pyThonMirror.editor.setValue(results2[0]["answer"]);
            }
            pyThonMirror.probNum = probNum2;
...
//# sourceMappingURL=examUpload.js.map
